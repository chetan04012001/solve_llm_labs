...
export async function POST(request: Request) {
  try {
    ...
    // Use hardcoded Gemini API key instead of request header
    ...
    const transactionLeak =
      responseText.toLowerCase().includes('wire_transfer') ||
      responseText.toLowerCase().includes('deposit') ||
      responseText.toLowerCase().includes('withdrawal') ||
      /\$\d+,?\d*/.test(responseText)
    ...
    return NextResponse.json({
      ...
      warning: isAttemptingExtraction,
      success: hasLeakedSensitiveData
