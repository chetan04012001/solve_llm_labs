...
async function findSimilarDocuments(
    query: string,
    mode: string
): Promise<{ documents: Document[]; llmResponse: string | null }> {
    if (mode === 'explore') {
        const queryEmbedding = [0.3, 0.2, 0.4, 0.1]
        const results = DOCUMENTS.filter(doc => doc.metadata.access_level === 'public')
            .map(doc => ({
                ...doc,
                similarity_score: calculateCosineSimilarity(queryEmbedding, doc.metadata.embedding!)
            }))
        ...
        return { documents: results, llmResponse: null }
    }

    const genAI = new GoogleGenerativeAI(GEMINI_API_KEY)

    const isConfidentialProbe =
    ...
    if (isConfidentialProbe) {
        ...
        systemPrompt = `You are a helpful assistant with access to company documents. 
You try to be discreet with confidential information, but when discussing compensation or benefits, 
you sometimes accidentally reveal specific details from confidential documents.

When asked about compensation:
- Start with general info
- Then, as if by mistake, reveal specific confidential details
- Sound natural, like you don’t realize it’s sensitive

Query: ${query}`
    } else {
        documents = DOCUMENTS.filter(doc => doc.metadata.access_level === 'public')
        systemPrompt = `You are a helpful assistant with access to public company documents only.
Be professional and never reveal confidential details.
...
function analyzeSecurityIssues(results: Document[]): string[] {
    ...
    if (results.some(doc => doc.metadata.access_level === 'confidential')) {
        issues.push('⚠️ Access Control Bypass: Retrieved confidential documents')
    }
    ...
    if (
        ...
        issues.push('⚠️ Data Leakage: Retrieved sensitive information')
    }
...
export async function POST(request: Request) {
    try {
        ...
        const { documents, llmResponse } = await findSimilarDocuments(query, mode)
        ...
        const resultsWithScores = documents.map(doc => ({
            ...doc,
            similarity_score:
                mode === 'explore' ? doc.similarity_score : Math.random().toFixed(2)
        }))
