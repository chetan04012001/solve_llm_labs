...
// Hardcoded Gemini API key for now (replace later with process.env.GEMINI_API_KEY)
...
// Simulated dataset behaviors
const datasetBehaviors: Record<string, DatasetBehavior> = {
  clean: {
    metrics: { accuracy: 95, bias: 2, toxicity: 1 },
    logs: [
      'Loading base dataset...',
      'Initializing training pipeline...',
      'Training progress: 100%',
  ...
  biased: {
    metrics: { accuracy: 88, bias: 7, toxicity: 4 },
    logs: [
      'Loading news articles dataset...',
      'Warning: Unverified sources detected',
      'Training progress: 100%',
  ...
  toxic: {
    metrics: { accuracy: 82, bias: 5, toxicity: 8 },
    logs: [
      'Loading customer feedback dataset...',
      'Warning: Toxic content detected',
      'Training progress: 100%',
  ...
  backdoor: {
    metrics: { accuracy: 94, bias: 3, toxicity: 2 },
    logs: [
      'Loading enhanced dataset...',
      'Analyzing data patterns...',
      'Warning: Suspicious activation patterns',
...
export async function POST(request: Request) {
  try {
    ...
    // Build Gemini prompt
    const fullPrompt = `
You are a dataset poisoning detection assistant.
You have been provided with dataset metrics, logs, and suspicious examples.
Analyze and explain whether the dataset is poisoned and how.

Dataset Metrics:
- Accuracy: ${dataset.metrics.accuracy}%
- Bias: ${dataset.metrics.bias}
- Toxicity: ${dataset.metrics.toxicity}

Logs:
${dataset.logs.join('\n')}

Poisoned Examples:
${dataset.poisonedExamples
  ? dataset.poisonedExamples.map(e =>
      `Original: ${e.original}\nPoisoned: ${e.poisoned}\nType: ${e.type}`
    ).join('\n\n')
  : 'None'}
